...
SWAYSOCK="/tmp/sway.sock";
     ...
     postStart = "${pkgs.coreutils}/bin/env";
  #   script = "${pkgs.sway}/bin/sway -c ${sway_cfg}";
     script = "${pkgs.river}/bin/river";
  #   script = "${pkgs.kitty}/bin/kitty";
      serviceConfig = {
        PassEnvironment = "SALAM XDG_RUNTIME_DIR WAYLAND_DISPLAY XDG_SESSION_TYPE";
        User = "svr";
     #   Group = "pulse-access";
        Restart = "on-failure";
     #   TimeoutSec = 3;
...
#          environment.variables = { 
 ...
 #          };
     ...
     #           command = "/run/current-system/sw/bin/bash -c ${pkgs.river}/bin/river";
