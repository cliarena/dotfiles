...
in {
  ...
  resource.vault_pki_secret_backend_root_sign_intermediate.consul_int_signed_csr = {
    depends_on = ["vault_pki_secret_backend_intermediate_cert_request.consul_int_csr"];
    backend = pki.path;
    csr = "\${vault_pki_secret_backend_intermediate_cert_request.consul_int_csr.csr}";
    common_name = "${consul_domain} Intermediate Certificate";
    exclude_cn_from_sans = true;
    # revoke = true;
    format = "pem_bundle";
  ...
  resource.vault_pki_secret_backend_root_sign_intermediate.nomad_int_signed_csr = {
    depends_on = ["vault_pki_secret_backend_intermediate_cert_request.nomad_int_csr"];
    backend = pki.path;
    csr = "\${vault_pki_secret_backend_intermediate_cert_request.nomad_int_csr.csr}";
    common_name = "${nomad.domain} Intermediate Certificate";
    exclude_cn_from_sans = true;
    # revoke = true;
    format = "pem_bundle";
  ...
  resource.vault_pki_secret_backend_intermediate_set_signed.consul_int_csr = {
    ...
    backend = pki_int.path;
    certificate = "\${vault_pki_secret_backend_root_sign_intermediate.consul_int_signed_csr.certificate}";
  };
  resource.vault_pki_secret_backend_intermediate_set_signed.nomad_int_csr = {
    ...
    backend = pki_int.path;
    certificate = "\${vault_pki_secret_backend_root_sign_intermediate.nomad_int_signed_csr.certificate}";
  };
